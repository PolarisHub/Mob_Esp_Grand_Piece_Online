...
function EspObject.new(mob, interface)
    local self = setmetatable({}, EspObject);
    self.mob = assert(mob, "Missing argument #1 (Model expected)");
...
function EspObject:Construct()
    ...
    self.drawings = {
        ...
        visible = {
            ...
            name = self:_create("Text", { Text = self.mob.Name, Center = true, Visible = false }),
...
function EspObject:Update()
    ...
    self.options = interface.settings;
    self.character = self.mob;
    self.health, self.maxHealth = interface.getHealth(self.mob);
    self.weapon = interface.getWeapon(self.mob);
    self.enabled = self.options.enabled and self.character and not
        (#interface.whitelist > 0 and not find(interface.whitelist, self.mob.Name));
...
function ChamObject.new(mob, interface)
...
function ChamObject:Update()
    ...
    local interface = self.interface;
    local character = self.mob;
    local options = interface.settings;
...
function EspInterface.Load()
    ...
    local function createObject(mob)
        EspInterface._objectCache[mob] = {
            EspObject.new(mob, EspInterface),
            ChamObject.new(mob, EspInterface)
        };
    end

    local function removeObject(mob)
        local object = EspInterface._objectCache[mob];
        if object then
            for i = 1, #object do
                object[i]:Destruct();
            end
            EspInterface._objectCache[mob] = nil;
        end
    end

    -- Find all existing NPCs in workspace.NPCs
    ...
    if npcs then
        ...
        EspInterface.npcAdded = npcs.ChildAdded:Connect(function(child)
            if child:IsA("Model") then
                createObject(child);
            end
        end);

        EspInterface.npcRemoved = npcs.ChildRemoved:Connect(function(child)
            if EspInterface._objectCache[child] then
                removeObject(child);
            end
        end);
    end
...
function EspInterface.Unload()
    ...
    if EspInterface.npcAdded then
        EspInterface.npcAdded:Disconnect();
    end
    if EspInterface.npcRemoved then
        EspInterface.npcRemoved:Disconnect();
    end
...
function EspInterface.getWeapon(mob)
    return "Unknown";
end

function EspInterface.getHealth(mob)
    local humanoid = mob and findFirstChildOfClass(mob, "Humanoid");
    if humanoid then
